...
class LinkedInScreen extends StatefulWidget {
  ...
  LinkedInScreen(this.currentUri, this.currentPageUrl, {Key? key}) : super(key: key);

  @override
  State<LinkedInScreen> createState() => _LinkedInScreenState();
}

class _LinkedInScreenState extends State<LinkedInScreen> with
    AutomaticKeepAliveClientMixin<LinkedInScreen>{
  late InAppWebViewController _webViewController;


  @override
  void didUpdateWidget(LinkedInScreen oldWidget) {
    ...
    if (oldWidget.currentPageUrl != widget.currentPageUrl) {
      ...
      print("LinkedInScreen didUpdateWidget :   :didUpdateWidget Method Called  ||||||||||||||||||    |||||||||||||||||||| called } ");
    }
  }
  ...
  Widget build(BuildContext context) {
    super.build(context);
    print("LinkedInScreen Build Method Called  ||||||||||||||||||    |||||||||||||||||||| called } ");
    print("LinkedInScreen Build Method Called  |||||||||||||||||| Opening ${widget.currentPageUrl}    |||||||||||||||||||| called } ");
